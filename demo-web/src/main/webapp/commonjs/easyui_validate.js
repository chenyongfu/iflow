...
$.extend($.fn.validatebox.defaults.rules, {
	...
	integelengthexpand : {// 验证整数,长度最长2位
		validator : function(value) {
			return /^\d{1,2}$/i.test(value);
		},
		message : '请输入整数,长度不能超过两位'
	},
	age : {// 验证年龄
		validator : function(value) {
    ...
    phoneOrMobile : {// 验证电话号码
		validator : function(value) {
			var phoneRex = /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/;
			var mobileRex =  /^(13|15|18)\d{9}$/;
			return phoneRex.test(value) || mobileRex.test(value);
		},
		message : '请输入正确格式的固定电话或手机号码'
	}
});